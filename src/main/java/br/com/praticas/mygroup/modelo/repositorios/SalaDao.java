...
public class SalaDao implements ISalaDao {
    ...
    public List<Sala> listarTodos() {
        EntityManager em = JPAUtil.getEntityManager();
        List<Sala> salas = em.createQuery("SELECT s FROM Sala s WHERE s.ativo = 'TRUE'", Sala.class).getResultList();
        em.close();
        return salas;
    }
    ...
    public List<Sala> buscarSalasPorTitulo(String titulo) {
        List<Sala> salas;
        EntityManager em = JPAUtil.getEntityManager();
        Query query = em.createQuery("SELECT s FROM Sala s WHERE s.titulo LIKE :titulo AND s.ativo = 'TRUE'", Sala.class);
    ...
    public List<Usuario> buscarUsuariosNaoSala(Sala s) throws DaoException {
        List<Usuario> users;
        EntityManager em = JPAUtil.getEntityManager();
        Query query = em.createNativeQuery("SELECT u.* FROM usuario u WHERE u.ativo = 'TRUE' AND u.id NOT IN(SELECT s.usuarios_id FROM sala_usuario s WHERE u.id = s.usuarios_id AND s.salasParticipando_id = :idSala)", Usuario.class);
    ...
    public List<Sala> buscarMinhasSalas(Usuario usuario) throws DaoException {
        List<Sala> salas;
        EntityManager em = JPAUtil.getEntityManager();
        Query query = em.createQuery("SELECT s FROM Sala s WHERE s.usuarioPrincipal.id = :id AND s.ativo = 'TRUE'", Sala.class);
    ...
    @Override
    public void removerUsuarioSala(Usuario usuario, Sala sala) throws DaoException {
        EntityManager em = JPAUtil.getEntityManager();
        Query query = em.createNativeQuery("DELETE FROM sala_usuario WHERE salasParticipando IN :idSala AND usuarios_id IN :idUsuario");
